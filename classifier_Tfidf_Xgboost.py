...
# Constants
MODEL_DIR = "Assets/Outputs/Models/XGBoost"
...
class LanguageClassifierTFIDF:
    """
    A language classifier using TF-IDF for feature extraction and XGBoost for classification.
    """

    def __init__(self, max_features: int = 1000, test_size: float = 0.3, random_state: int = 42):
        """
        ...
        self.pipeline = Pipeline([
            ...
            ('clf', XGBClassifier(n_estimators=500, max_depth=6, learning_rate=0.1, random_state=random_state))
        ])
    ...
    def evaluate(self, X: List[str], y: List[int]) -> None:
        ...
        evaluation_report_path = os.path.join(EVALUATION_DIR, 'evaluation_report_XGBoost.txt')
        try:
...
def main():
    parser = argparse.ArgumentParser(description='Language Classification with TF-IDF and XGBoost')
